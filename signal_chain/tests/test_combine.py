...
def test_aligned_boost():
    e = combine([_sig("trend", Direction.BUY, 0.5),
                 _sig("research", Direction.BUY, 1.0)], as_of=D0)
...
def test_partial_with_neutral():
    e = combine([_sig("trend", Direction.BUY, 0.6),
                 _sig("research", Direction.NEUTRAL, 0.0)], as_of=D0)
    assert e.agreement == "partial"
    assert abs(e.conviction - 0.3) < 1e-6


def test_conflicted():
    e = combine([_sig("trend", Direction.BUY, 0.8),
                 _sig("research", Direction.SELL, -0.6)], as_of=D0)
    assert e.agreement == "conflicted"
    assert abs(e.conviction - 0.1) < 1e-6


def test_single_source_discount():
    e = combine([_sig("trend", Direction.BUY, 1.0)], as_of=D0)
    assert e.agreement == "single_source"
    assert abs(e.conviction - 0.7) < 1e-6


def test_asof_gap_degrades_older():
    old = _sig("trend", Direction.SELL, -0.8, as_of=D0 - timedelta(days=5))
    new = _sig("research", Direction.BUY, 1.0)
    e = combine([old, new], as_of=D0, max_asof_gap_days=1)
    degraded = [c for c in e.components if c.degraded]
    assert len(degraded) == 1 and degraded[0].engine == "trend"
    assert e.agreement == "single_source"  # 旧信号降级后只剩一条有效


def test_insufficient_peer_becomes_single_source():
    weak = _sig("trend", Direction.NEUTRAL, 0.0)
    weak = weak.model_copy(update={
        "data_status": "insufficient_data",
        "degraded": True,
        "data_gaps": ["报价 missing: 无报价"],
    })
    e = combine([weak, _sig("research", Direction.BUY, 1.0)], as_of=D0)
    assert e.agreement == "single_source"
    assert abs(e.conviction - 0.7) < 1e-6
    assert e.quality_notes and "trend" in e.quality_notes


def test_opinion_still_votes_with_peer():
    view = _sig("trend", Direction.NEUTRAL, 0.0).model_copy(update={
        "data_status": "opinion",
        "data_gaps": ["报告标明关键数据缺失: 行情缺失"],
    })
    e = combine([view, _sig("research", Direction.BUY, 0.5)], as_of=D0)
    assert e.agreement == "partial"
...
def test_both_opinions_are_not_dual_price_confirmation():
    a = _sig("trend", Direction.BUY, 0.4).model_copy(update={"data_status": "opinion"})
    b = _sig("research", Direction.BUY, 0.6).model_copy(update={"data_status": "opinion"})
...
def test_both_insufficient_does_not_vote():
    ...
    e = combine([abstain("trend"), abstain("research")], as_of=D0)
